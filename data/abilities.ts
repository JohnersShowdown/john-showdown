...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
    ...
    onTryHit(target, source, move) {
      if (target !== source && move.flags["wind"]) {
        if (!this.boost({ spa: 1 }, target, target)) {
          this.add("-immune", target, "[from] ability: Wind Charge");
        }
        return null;
      }
    },
	...
	caffeinerush: {
		...
		if (pokemon.hp <= pokemon.maxhp / 2) {
			...
			this.add('-activate', pokemon, 'ability: Caffeine Rush');
	...
	ancientforce: {
		...
		for (const condition of sideConditions) {
			const sideCondition = pokemon.side.sideConditions[condition];
			if (!sideCondition) continue;
			// Layered hazards
			if (sideCondition.layers) {
				boosts += sideCondition.layers;
			} else {
				// Single-layer hazards
				boosts++;
			}
	    	}
	    	if (boosts > 0) {
			this.add('-activate', pokemon, 'ability: Ancient Force');
	...
	this.add('-activate', 'ability: Moxie');
	this.add('-activate', 'ability: Adrenaline Rush');
	this.add('-activate', 'ability: Charisma');
	this.add(
		'-formechange',
		pokemon,
		targetSpecies.name,
